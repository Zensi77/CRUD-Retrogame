...
@carritoRoutes.post("", status_code=status.HTTP_201_CREATED, response_model=Cart)
def add_carrito(register: Cart, db = Depends(get_db), current_user = Depends(current_user)):
    nuevo = CartDB(**register.dict(), userId=current_user.id)
    db.add(nuevo)
    db.commit()
    db.refresh(nuevo)
...
def modify_cart(cart: Cart, db = Depends(get_db), current_user = Depends(current_user)):
    registro = db.query(CartDB).filter(CartDB.articuloId == cart.articuloId and CartDB.userId == current_user.id).first()
    if not registro:
...
@carritoRoutes.delete("/{id}", status_code=status.HTTP_200_OK, response_model=dict[str, str])
def delete_item(id_item: int, db = Depends(get_db), current_user = Depends(current_user)):
    registro = db.query(CartDB).filter(CartDB.articuloId == id_item).all()
    if registro: 
        db.delete(registro)
        db.commit()
...
def pedido(db = Depends(get_db), current_user = Depends(current_user)):
    lista = db.query(CartDB).filter(CartDB.userId == current_user.id).all()
    if lista:
        for item in lista:
            db.delete(item)
        db.commit()
